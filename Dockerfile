...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-vsphere/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omni-infra-provider-vsphere -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omni-infra-provider-vsphere-linux-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-vsphere/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omni-infra-provider-vsphere -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omni-infra-provider-vsphere-linux-arm64
