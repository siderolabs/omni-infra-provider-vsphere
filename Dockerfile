...
#
# Generated on 2026-05-30T15:34:35Z by kres 8085f7d.
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-vsphere/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /omni-infra-provider-vsphere-linux-amd64

# builds omni-infra-provider-vsphere-linux-arm64
FROM base AS omni-infra-provider-vsphere-linux-arm64-build
COPY --from=generate / /
WORKDIR /src/cmd/omni-infra-provider-vsphere
ARG GO_BUILDFLAGS
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-vsphere/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /omni-infra-provider-vsphere-linux-arm64
...
FROM scratch AS omni-infra-provider-vsphere-linux-arm64
COPY --from=omni-infra-provider-vsphere-linux-arm64-build /omni-infra-provider-vsphere-linux-arm64 /omni-infra-provider-vsphere-linux-arm64
...
COPY --from=omni-infra-provider-vsphere-linux-arm64 / /
