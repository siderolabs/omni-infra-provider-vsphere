...
#
# Generated on 2026-09-16T15:24:47Z by kres 3e1d3c0.
...
.PHONY: $(ARTIFACTS)/omni-infra-provider-vsphere-linux-arm64
$(ARTIFACTS)/omni-infra-provider-vsphere-linux-arm64:
	@$(MAKE) local-omni-infra-provider-vsphere-linux-arm64 DEST=$(ARTIFACTS)

.PHONY: omni-infra-provider-vsphere-linux-arm64
omni-infra-provider-vsphere-linux-arm64: $(ARTIFACTS)/omni-infra-provider-vsphere-linux-arm64  ## Builds executable for omni-infra-provider-vsphere-linux-arm64.

.PHONY: omni-infra-provider-vsphere
omni-infra-provider-vsphere: omni-infra-provider-vsphere-linux-amd64 omni-infra-provider-vsphere-linux-arm64  ## Builds executables for omni-infra-provider-vsphere.
...
.PHONY: sign-image-omni-infra-provider-vsphere
sign-image-omni-infra-provider-vsphere:  ## Signs the image for omni-infra-provider-vsphere. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	...
	@TMP=$$(mktemp -d) && trap 'rm -rf "$$TMP"' EXIT && \
	  ...
	  docker run --rm -p 127.0.0.1:8585:8585 \
	    ...
	    $(REGISTRY)/$(USERNAME)/omni-infra-provider-vsphere:$(IMAGE_TAG)
